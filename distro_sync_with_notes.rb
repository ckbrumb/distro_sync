...
# add error checking
# add a conditional that will skip a distribution point during syncing if it cannot be reached

# General code clean up. Turn whatever repeatable blocks into methods or procs.

# Add a section to leverage the API to report to the JSS that syncing has been successful.

# Add Windows functionality

# Use IO to get output from rsync during the syncing process

# Set up a way to automate the authorized_keys process

# Set up to run the perform_sync automatically via a LaunchDaemon and Cron job

# In delete_servers need to make sure the server doesn't exists if the string doesn't match. (DONE)
...
class RubyRsyncBuilder
	...
	def delete_server
		...
		if File.readlines(@filename).grep(/#{Regexp.escape(server_to_delete)}/).size > 0
			...
			puts "#{server_to_delete} does not exist"
			delete_server
		end
	end
	...
	def perform_sync
		...
		File.foreach(@filename) do |line|
			...
			# add the ping that will make sure the sever is available.
			# remote_serv = line.chomp
